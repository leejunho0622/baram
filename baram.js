window.onload = () => {
    ...
    function interaction(px) {
        entity = ["mantis", "scorpion", "rabbit", "npc1", "npc2"];
        for(let i=0; i<entity.length; i++){
            const entityXY = document.getElementById(entity[i]).getBoundingClientRect();
            if(px.top - 50 === entityXY.top || px.left + 50 === entityXY.left || px.top + 50 === entityXY.top || px.left - 50 === entityXY.left){
                const log = document.querySelector("#status p");
                text += entity[i]+"와 대화를 시도합니다.<br>";
                log.innerHTML = `${text}`;
            }
        }
    }
